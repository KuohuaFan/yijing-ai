...
export async function createBaziResearchGuide(input: BaziInput, question?: string) {
  ...
  if (isBaziHighRiskQuestion(question)) {
    return {
      ...
      content: `## 1. 計算定位\n本次仍只保留四柱、節氣月、日界與指定流年的確定性計算；不把盤面轉成個人事件預估。\n\n## 2. 盤面結構\n可以比較日主、月令、藏干、十神及可見五行，但不據此判定疾病、訴訟、投資報酬、破財或其他吉凶結果。\n\n## 3. 傳統術語\n傳統術語在此作文化與結構標記；不同流派可能有不同口徑，不能替代可查證事實或專業判斷。\n\n## 4. 流年並讀\n流年與大運只作時間軸定位，不提供某年會發生何事、成功機率或行動指令。\n\n## 5. 慣例與不確定性\n可比較子初／子正、節氣月與起運口徑；真太陽時尚未套用，出生時間與地點仍須回到可靠來源核對。\n\n## 6. 研究問題與限制\n可改問「本盤的日主、月令、十神與藏干如何作結構閱讀？」重大決策應依可查證事實、專業意見與風險評估處理。`,
      chart,
    };
  }
